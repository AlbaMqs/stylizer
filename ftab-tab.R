...
# Thème personnalisé pour flextable
ftab_theme_perso <- function(ft) {
  
  # Mise en forme des en-têtes
  ft <- bold(ft, part = "header")  # En-têtes en gras
  ft <- align(ft, align = "center", part = "header")  # Centrer le texte des en-têtes
  ft <- fontsize(ft, size = 10.5, part = "header")  # Taille de police pour l'en-tête
  ft <- font(ft, fontname = "Times New Roman", part = "header")
  
  # Mise en forme du corps du tableau
  ft <- fontsize(ft, size = 10, part = "body")  # Taille de police plus petite pour le corps
  ft <- font(ft, fontname = "Times New Roman", part = "body")  # Police Times New Roman
  ft <- align(ft, align = "center", part = "body")  # Centrer le contenu des cellules
  ft <- valign(ft, valign = "center", part = "body")  # Alignement vertical centré
  
  # Mise en page de la colonne des modalités
  ft <- align(ft, align = "left", part = "body", j = 1)  # Centrer le contenu des cellules
  
  # Mise en forme du pied du tableau
  ft <- italic(ft, part = "footer")  # Texte en italique dans le footer
  ft <- fontsize(ft, size = 9, part = "footer")  # Taille de police réduite pour le footer
  ft <- line_spacing(ft, space = 0.8, part = "footer")  # Réduction de l'espacement entre lignes dans une cellule
  ft <- padding(
    ft,
    padding.top = 1,
    padding.bottom = 1,
    part = "footer"
  )  # Ajuste l'espacement entre cellules du footer
  
  # Ajuster les bordures
  thick_border <- fp_border(color = "black", width = 1.5)
  
  ft <- hline_top(ft, part = "header", border = thick_border)  # Ligne épaisse au-dessus de l'en-tête
  ft <- hline_bottom(ft, part = "header", border = thick_border)  # Ligne fine sous l'en-tête
  ft <- hline_bottom(ft, part = "body", border = thick_border)  # Ligne épaisse sous le tableau
  # 
  # Ajuster les marges des cellules pour aérer un peu plus le tableau
  ft <- padding(
    ft,
    padding.top = 2,
    padding.bottom = 2,
  ...
  # Mettre le padding droit et gauche à 0 pour toutes les colonnes sauf la première
  ...
  # pvalue_columns <- which(str_detect(ft$col_keys, regex("p-value", ignore_case = TRUE)))
  # 
  # # Étape 2 : Appliquer le fond rouge sur les colonnes identifiées
  # ft <- bg(
  #   ft,
  #   i = NULL,
  #   j = pvalue_columns,
  #   bg = "red",
  #   part = "body"                 # Applique la couleur de fond au corps du tableau
  # )
  
  # Ajustements des largeurs de colonnes pour s'adapter au contenu
  ft <- autofit(ft)
